...
def extract_info_from_path(path):
    """Extract take_id and camera name from the path."""
    parts = path.split('//')
    if len(parts) >= 3:
        take_id = parts[1]
        camera_name = parts[2].split('/')[0]
        return take_id, camera_name
    return None, None
...
def main(input_file_name, output_file_name):
    ...
    # Group pairs by (take_id, ego_camera, exo_camera)
    grouped_pairs = defaultdict(list)
    
    for pair in all_pairs:
        # Extract camera info from both paths
        take_id1, camera1 = extract_info_from_path(pair[0])
        take_id2, camera2 = extract_info_from_path(pair[2])
        
        if take_id1 == take_id2 and take_id1:
            # Determine ego/exo based on "aria" in camera name
            if "aria" in camera1.lower():
                key = (take_id1, camera1, camera2)
                grouped_pairs[key].append(pair)
            elif "aria" in camera2.lower():
                # Reorder to keep ego first
                key = (take_id1, camera2, camera1)
                grouped_pairs[key].append([pair[2], pair[3], pair[0], pair[1]])
    
    # Get unique camera pairs
    unique_pairs = list(grouped_pairs.keys())
    print(f"Unique (take_id, ego_camera, exo_camera) combinations: {len(unique_pairs)}")
    ...
    for i, key in enumerate(selected_pairs[:10]):
        ...
        print(f"  {i+1}. Take: {take_id[:8]}..., Ego: {ego_cam}, Exo: {exo_cam}")
...
if __name__ == "__main__":
    ...
    main("val_exoego_pairs.json", "custom_val_egoexo_pairs_10.json")
